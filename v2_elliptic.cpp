...
void rho_generator_p(blitz::Array<double,2>& rho)
{
    ...
    for(int i=0; i<size_x; i++)
    {
        for(int j=0; j<size_x; j++)
        {
            ...
            rho(i,j) = exp(-ellipse) * Gaussian_p();
        }
    }
}
        ...
        for(int i1=0; i1<size_x; i1=i1+1)
        {
            ...
            for(int j1=0; j1<size_x; j1=j1+1)
            {
                ...
                if( (k1-(K-0.5*dK) ) * ( k1-(K+0.5*dK) )  <0.0 )
                {
					...
					//cerr << K << " "<< phi_1 << " " << cos(2*phi_1) << " " << (x2(kx1_t)- x2(ky1_t))/(x2(kx1_t) + x2(ky1_t)) << " " << (x2(kx1_t)- x2(ky1_t))/k12  << "\n"; 
					
				

					v0 += real(k1*dK*amp) ;
					v1 += k1*dK*amp*exp(1.0*phi_1*cd(0.0,1.0)) ;	 
					v2 += k1*dK*amp*exp(2.0*phi_1*cd(0.0,1.0)) ;
					v3 += k1*dK*amp*exp(3.0*phi_1*cd(0.0,1.0)) ;
     	...
     	<< " " << real(v4*conj(v4))/x2(v0) //8
     	<< " " << real(v1*conj(v1))/x2(v0) //9 
     	<< " " << real(v3*conj(v3))/x2(v0) //10
     	<< "\n").toString() << flush;
    }
}
