...
// ----------------------------------------------------------------------------
{
  ...
  complex<double> f;     // Intermediate storage
  ...
  Q(2,0) = real(A(0,0))*real(A(1,1)) - Q(2,1);
  norm    = SQR_ABS(Q(0,0)) + SQR_ABS(Q(1,0)) + SQR(real(Q(2,0)));
  n0      = n0tmp + SQR(real(A(0,0)));
  n1      = n1tmp + SQR(real(A(1,1)));
  error   = n0 * n1;
  ...
  if (fabs(t) > 8.0 * DBL_EPSILON * wmax)
  {
    ...
    Q(2,1)  = real(A(0,0))*real(A(1,1)) - real(Q(2,1));
  ...
  else
  {
    ...
    {
      for (j=0; j < 3; j++)
        if (real(Q(j,0)) != 0.0  ||  imag(Q(j,0)) != 0.0) // Find nonzero element of v(0) ...
